...
- [ ] 2. 数据层：Drizzle schema 与迁移
  - [ ] 2.1 定义 Drizzle TS schema
...
- [ ] 3. 认证、会话与邮箱验证（后端）
  ...
  - [ ] 3.4 实现 SES 发信服务与 EmailVerificationService
  ...
  - [ ] 3.13 实现认证中间件、角色 Guard 并挂载 /auth 路由
...
- [ ] 5. 商品浏览、搜索与商品管理（后端）
  - [ ] 5.1 实现 CatalogService 列表/搜索/详情
  ...
  - [ ] 5.6 实现 AdminProductService 创建/编辑/上下架
...
- [ ] 6. 图片上传、商品图集与头像（后端）
  ...
  - [ ] 6.2 实现 UploadService.presign 校验与鉴权
  ...
  - [ ] 6.4 实现 ProductImageService 图集管理
...
- [ ] 11. 积分、兑换历史与员工列表（后端）
  ...
  - [ ] 11.5 实现 PointsService 批量调整（部分成功）
...
- [ ] 18. 检查点 — 确保前端与 i18n 测试通过
  - 确保所有测试通过，如有疑问请询问用户。

- [ ] 19. 基础设施编排（CDK/SAM，TypeScript）
  - [ ] 19.1 搭建 IaC 骨架 + 前端 S3 + CloudFront
    - CDK/SAM stack；前端私有 S3（OAC）+ CloudFront 默认行为回源；SPA 深链接 403/404 → `/index.html`（200）
    - _Requirements: 19.3_
  - [ ] 19.2 定义 API Gateway + 单体 Lambda
  ...
  - [ ] 19.3 定义 RDS PostgreSQL（公网可达）
    - public subnet + `publiclyAccessible`；安全组仅放行受限来源 IP + DB 端口；强口令
    - _Requirements: 19.3, 20.5_
  - [ ] 19.4 定义上传桶 + CloudFront /media 行为 + CORS
...
```json
{
  "waves": [
    ...
    { "id": 9, "tasks": ["6.3", "6.5", "6.7", "7.2", "7.4", "8.1"] },
    { "id": 10, "tasks": ["8.2", "8.3", "8.4", "8.6", "8.9"] },
    { "id": 11, "tasks": ["8.5", "8.7", "8.8", "8.10", "10.1", "10.4", "11.1", "11.5", "11.7", "11.9", "12.1", "12.3"] },
    ...
    { "id": 18, "tasks": ["20.3", "21.1", "21.2", "21.3", "21.4", "21.5", "21.6", "21.7", "21.8"] }
  ]
}
```
