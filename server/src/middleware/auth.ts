...
import { DrizzleSessionService, type SessionManager } from '../services/session-service'
...
export interface AuthenticatedUser {
  ...
  /** 角色（授权用，来自 JWT `role`）。 */
  role: Role
  /** 会话 id（登出/空闲校验用，来自 JWT `sid`）。 */
  sessionId: string
}

// Express `Request` 上挂载认证主体（经 authMiddleware 填充）。
declare global {
...
export class JwtHs256Verifier implements JwtVerifier {
  ...
  verify(token: string): AuthTokenPayload | null {
    if (!this.secret) {
      throw new Error(
        'JWT_SECRET is not set. A verification secret is required to authenticate requests.',
      )
    }
    try {
      const decoded = jwt.verify(token, this.secret)
      if (typeof decoded !== 'object' || decoded === null) return null
      const { sub, sid, role } = decoded as Record<string, unknown>
...
/** `createAuthMiddleware` 构造选项。 */
export interface AuthMiddlewareOptions {
  /** JWT 校验器（缺省基于 `JWT_SECRET` 的 HS256）。 */
  verifier?: JwtVerifier
  /** 会话管理器（缺省 Drizzle）；用于空闲校验与活跃刷新。 */
  sessionManager?: SessionManager
  /** 时钟（缺省 `() => new Date()`），便于测试控制会话过期判定。 */
  now?: () => Date
}
