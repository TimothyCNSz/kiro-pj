...
import { ERROR_DEFINITIONS, type ErrorCode } from './errors'

/** 通用 API 响应结构（与前端 `src/types/index.ts` 对齐）。 */
...
/** 分页参数（与前端 `src/types/index.ts` 对齐）。 */
export interface PaginationParams {
  page: number
  pageSize: number
}

/** 分页响应（与前端 `src/types/index.ts` 对齐）。 */
...
export function paginated<T>(
  ...
  return success(
    { list, total: meta.total, page: meta.page, pageSize: meta.pageSize },
    message,
  )
}
 ...
 */
export function failure(code: ErrorCode, message?: string): ApiResponse<null> {
  const def = ERROR_DEFINITIONS[code]
  return { code: def.appCode, message: message ?? code, data: null }
}

/** 判断响应是否为成功（`code === 0`）。 */
export const isSuccess = <T>(res: ApiResponse<T>): boolean => res.code === SUCCESS_CODE
