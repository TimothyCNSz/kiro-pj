...
import { CdkService, normalizeCdkCodes, type CdkGateway } from './cdk-service'
...
class FakeCdkGateway implements CdkGateway {
  /** productId -> product type (null-able via absence). */
  readonly types: Record<string, ProductType>
  /** productId -> inserted available codes. */
  readonly inserted: Record<string, string[]> = {}

  constructor(types: Record<string, ProductType> = {}) {
    this.types = types
  }

  async getProductType(productId: string) {
    return this.types[productId] ?? null
  }

  async insertCdks(productId: string, codes: string[]) {
...
describe('normalizeCdkCodes', () => {
  ...
  it('ignores non-string entries', () => {
    expect(normalizeCdkCodes(['A', 1, null, undefined, {}, 'B'] as unknown[])).toEqual(['A', 'B'])
  })
