 ...
 */
class FakeRepo implements CatalogProductRepository {
  constructor(
    private readonly products: Product[] = [],
    private readonly images: ProductImage[] = [],
  ) {}

  async listListed(_p: PaginationParams): Promise<ProductPage> {
    return { rows: [...this.products], total: this.products.length }
  }
  async searchListedByName(_keyword: string, _p: PaginationParams): Promise<ProductPage> {
    return { rows: [...this.products], total: this.products.length }
  }
...
describe('CatalogService.listProducts', () => {
  ...
  it('uses the injected stock resolver seam (task 5.4 virtual stock)', async () => {
    ...
    const stockResolver: StockResolver = (p) => (p.type === ProductType.Virtual ? 7 : p.stock)
...
describe('CatalogService.getProduct', () => {
  it('returns detail with type, description and gallery (主图在前) (需求 4.5)', async () => {
    const repo = new FakeRepo(
      [makeProduct({ id: 'p', name: 'Gadget', type: ProductType.Virtual, description: 'desc', pointsCost: 55, stock: 2 })],
      [
