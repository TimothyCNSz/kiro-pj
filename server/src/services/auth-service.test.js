...
import { AuthService, EMAIL_NOT_VERIFIED_MESSAGE, INVALID_CREDENTIALS_MESSAGE, ValidationError, } from './auth-service';
...
function makeService(overrides) {
    ...
    const issue = overrides?.issue ?? vi.fn(async () => ({ token: 't', expiresAt: new Date() }));
    const service = new AuthService({
        ...
        // 用可逆的假哈希以加速测试并保持确定性。
        hashPassword: overrides?.hashPassword ?? ((p) => `hash(${p})`),
        verifyPassword: overrides?.verifyPassword ?? ((p, s) => s === `hash(${p})`),
    });
...
describe('AuthService.register', () => {
    ...
    it('rejects a weak password with itemized VALIDATION field errors (1.1, 1.6)', async () => {
        ...
        await expect(service.register('ok@company.com', 'short')).rejects.toBeInstanceOf(ValidationError);
        try {
            await service.register('ok@company.com', 'short');
        }
        catch (err) {
            expect(err).toBeInstanceOf(ValidationError);
            expect(err.fieldErrors.password).toBe('WEAK_PASSWORD');
    ...
    it('rejects a non-company domain with a COMPANY_DOMAIN_REQUIRED field error (1.7)', async () => {
        ...
        try {
            ...
            expect(err).toBeInstanceOf(ValidationError);
            expect(err.fieldErrors.email).toBe('COMPANY_DOMAIN_REQUIRED');
        }
    });
    it('rejects a malformed email with an INVALID_EMAIL_FORMAT field error (1.6)', async () => {
        ...
        try {
            ...
            expect(err.fieldErrors.email).toBe('INVALID_EMAIL_FORMAT');
        }
    });
...
describe('AuthService.login', () => {
    async function seedActiveUser(store, email, password) {
    ...
    it('rejects a wrong password with the indistinguishable credentials error (1.14)', async () => {
        ...
        try {
            ...
            expect(err.message).toBe(INVALID_CREDENTIALS_MESSAGE);
        }
    });
    it('rejects an unknown email with the SAME error code and message as a wrong password (1.14)', async () => {
        ...
        expect(unknownErr.message).toBe(INVALID_CREDENTIALS_MESSAGE);
    });
    it('rejects a pending_verification account (correct credentials) with EMAIL_NOT_VERIFIED (1.13)', async () => {
        ...
        try {
            ...
            expect(err.message).toBe(EMAIL_NOT_VERIFIED_MESSAGE);
        }
    });
