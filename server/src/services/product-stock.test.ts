...
class FakeCdkCounter implements CdkCounter {
...
const virtualProduct = (id: string, stock = 999): StockProductView => ({
  id,
  type: ProductType.Virtual,
  stock, // cached value; must be ignored for virtual products
})

const physicalProduct = (id: string, stock: number): StockProductView => ({
  id,
