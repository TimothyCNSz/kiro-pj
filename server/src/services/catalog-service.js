...
import { ProductStatus } from '../lib/domain';
/**
...
export const defaultStockResolver = (product) => product.stock;
...
export class CatalogService {
     ...
     */
    async searchProducts(keyword, pagination) {
        const normalized = typeof keyword === 'string' ? keyword.trim() : '';
        const page = normalized.length === 0
    ...
    /** 防御式过滤 + 映射为列表页 DTO。 */
    async toListPage(page, pagination, keyword) {
        // 防御式不变式：无论仓储实现如何，输出仅含上架且（关键字为空或名称匹配）的商品（Property 8）。
        const filtered = page.rows.filter((p) => p.status === ProductStatus.Listed && nameMatches(p.name, keyword));
    ...
    async toListItem(product) {
        ...
        return {
            ...
            name: product.name,
            pointsCost: product.pointsCost,
            imageUrl: hasImage ? product.imageUrl : this.placeholderUrl,
...
export class DrizzleCatalogProductRepository {
    db;
    constructor(db = defaultDb) {
        this.db = db;
    }
    async listListed(pagination) {
    ...
    async searchListedByName(keyword, pagination) {
        const { limit, offset } = toLimitOffset(pagination);
