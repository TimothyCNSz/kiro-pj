...
import { OrderHistoryService, toLimitOffset, } from './order-history-service';
import { OrderStatus, OrderType } from '../lib/domain';
// ---------------------------------------------------------------------------
// In-memory fake repository (no real DB).
// ---------------------------------------------------------------------------
class FakeOrderHistoryRepository {
    /** orderId -> record. */
    ordersById = new Map();
    ...
    seedCdks(orderId, codes) {
        this.cdksByOrder.set(orderId, codes);
    }
    seedBalance(userId, balance) {
...
const USER = 'user-1';
function line(productName, quantity = 1, unitPoints = 100) {
    return { productId: `p-${productName}`, productName, quantity, unitPoints };
}
function physicalOrder(id, createdAt, overrides = {}) {
    return {
        id,
        userId: USER,
...
function virtualOrder(id, createdAt, overrides = {}) {
    return {
        id,
        userId: USER,
