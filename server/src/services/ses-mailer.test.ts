...
const asSesClient = (fake: FakeSesClient): SESClient => fake as unknown as SESClient
...
describe('SesMailer', () => {
  ...
  it('omits the HTML body when not provided', async () => {
    ...
    const input = (fake.commands[0] as SendEmailCommand).input
  ...
  it('throws when no from-address is configured', async () => {
    ...
    await expect(
      mailer.send({ to: 'e@corp.example', subject: 'S', text: 't' }),
