...
/** 一件待校验商品的请求数量与其当前可兑换库存。 */
export interface StockCheckItem {
  /** 商品 id。 */
  productId: string
  /** 商品名称（可选，用于更友好的库存不足提示）。 */
  name?: string
  /** 本次请求（加购后总量 / 结算数量）。 */
  requestedQuantity: number
  /** 该商品当前可兑换库存（虚拟=可用 CDK 数，实物=stock；由 StockResolver 解析）。 */
 ...
 */
export function isWithinStock(requestedQuantity: number, availableStock: number): boolean {
 ...
 */
export function findStockViolations(items: readonly StockCheckItem[]): StockCheckItem[] {
  return items.filter((item) => !isWithinStock(item.requestedQuantity, item.availableStock))
}

/** 生成某违规条目的库存不足 / 已兑完提示文案（需求 5.2, 6.3）。 */
export function stockViolationMessage(item: StockCheckItem): string {
 ...
 */
export function assertWithinStock(items: readonly StockCheckItem[]): void {
