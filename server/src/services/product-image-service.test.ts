...
import type { NewProductImage, ProductImage } from '../db/schema'
...
class FakeProductImageGateway implements ProductImageGateway {
  ...
  seed(rows: Array<Partial<ProductImage> & { productId: string }>): void {
    for (const r of rows) this.push(r)
  }

  private push(r: Partial<ProductImage> & { productId: string }): ProductImage {
  ...
  async clearPrimary(productId: string): Promise<void> {
    for (const r of this.rows) if (r.productId === productId) r.isPrimary = false
  }

  async markPrimary(productId: string, imageId: string): Promise<void> {
    for (const r of this.rows) {
      if (r.productId === productId && r.id === imageId) r.isPrimary = true
    }
  }
  ...
  primaryCount(productId: string): number {
...
describe('ProductImageService.addImage', () => {
  let gateway: FakeProductImageGateway
  ...
  it('rejects adding beyond the limit with IMAGE_LIMIT_EXCEEDED and leaves existing images intact (需求 22.11, 22.12)', async () => {
    for (let i = 0; i < 5; i++) await service.addImage(PID, `k${i}`, `u${i}`)
