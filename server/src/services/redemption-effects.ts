...
/**
 * 低库存提醒触发接缝（由 AlertService 实现，任务 12.3）。`handle` 为可选的事务/
 * 数据库句柄：兑换副作用传入本事务句柄以在同一事务内去重写入（需求 5.3、15.1）。
 * 以 `unknown` 松耦合具体持久化类型（与 {@link RedemptionTx.handle} 对齐）。
 */
export interface LowStockAlerter {
  triggerLowStock(productId: string, handle?: unknown): Promise<void>
}

/** {@link buildRedemptionApplyEffects} 依赖。 */
export interface RedemptionEffectsDependencies {
  /** 低库存提醒器（库存降为 0 时触发，去重，需求 5.3）。 */
  alertService: LowStockAlerter
  /**
   * 是否在兑换成功后移除本次兑换的购物车条目（需求 7.5）。
   * 结算（来自购物车）应为 `true`（默认）；立即兑换不来自购物车，应传 `false`。
   */
  removeRedeemedCartItems?: boolean
}
 ...
 */
export function buildRedemptionApplyEffects(
  deps: RedemptionEffectsDependencies,
): NonNullable<RedemptionHooks['applyEffects']> {
  const removeRedeemedCartItems = deps.removeRedeemedCartItems ?? true

  return async (ctx: RedemptionContext, tx: RedemptionTx): Promise<void> => {
    ...
    if (removeRedeemedCartItems) {
      await tx.removeCartItems(
        ctx.userId,
        ctx.lines.map((line) => line.productId),
      )
    }
