...
import type { RedemptionLine } from './redemption-service'

/**
 * 订单项草稿：下单时对商品的快照（与设计 Data Models 的 `OrderItem` 对齐）。
 * `name` / `unitPoints` 为下单时刻快照，保证历史订单展示稳定（不随商品后续改名/调价变化）。
 */
export interface OrderItemDraft {
  /** 商品 id。 */
  productId: string
  /** 下单时的商品名称快照（对应 `OrderItem.productName`）。 */
  name: string
  /** 兑换数量。 */
  quantity: number
  /** 下单时的单价快照：所需积分（对应 `OrderItem.unitPoints`）。 */
  unitPoints: number
}

/**
 * 订单草稿：一次兑换按类型拆分后得到的单个订单（尚未持久化）。
 * 与设计 Data Models 的 `Order` 对齐（`type` / `pointsSpent`），由任务 8.9 据此
 * INSERT 真实订单及其订单项。
 */
export interface OrderDraft {
  /** 订单类型：实物 / 虚拟（需求 7.9）。 */
  type: OrderType
  /** 该订单消耗积分 = 归属本订单的各订单项 `unitPoints × quantity` 之和。 */
  pointsSpent: number
  /** 归类到本订单的订单项。 */
...
const PRODUCT_TYPE_TO_ORDER_TYPE: Record<ProductType, OrderType> = {
  [ProductType.Physical]: OrderType.Physical,
  [ProductType.Virtual]: OrderType.Virtual,
}

// 草稿输出顺序固定为「实物在前、虚拟在后」，使结果稳定、便于测试与展示。
const ORDER_TYPE_OUTPUT_ORDER: readonly OrderType[] = [OrderType.Physical, OrderType.Virtual]
 ...
 */
export function splitOrdersByType(lines: readonly RedemptionLine[]): OrderDraft[] {
  // 以订单类型分组累积；使用 Map 保证同类型明细合并进同一份草稿。
  const draftsByType = new Map<OrderType, OrderDraft>()
