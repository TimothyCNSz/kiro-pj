...
/** `presign` 入参（见设计「后端 API 契约 · 上传」，需求 22.1）。 */
export interface PresignRequest {
  /** 上传用途：头像 或 商品图（需求 22.1）。 */
  purpose: UploadPurpose
  /** 目标实体 id：头像为 userId、商品图为 productId。用于鉴权与 objectKey。 */
  targetId: string
  /** 声明的图片 `Content-Type`（需求 22.2）。 */
  contentType: string
  /** 声明的单张图片字节数（需求 22.3）。 */
  size: number
}

/** `presign` 成功结果：直传 URL + 对象 key + 公开访问 URL。 */
export interface PresignResult {
  /** 短时效预签名 PUT URL，客户端直传 S3（需求 22.6, 22.7）。 */
  uploadUrl: string
  /** 目标对象 key。 */
  objectKey: string
  /** 公开访问 URL（CloudFront /media/*，需求 22.10）。 */
  publicUrl: string
}

/**
 * 本服务所依赖的 S3 预签名原语能力子集（结构化接口，便于注入测试替身）。
 * 与 {@link S3PresignService} 的方法签名对齐。
 */
export interface PresignBackend {
...
/** `UploadService` 构造选项（依赖可注入）。 */
export interface UploadServiceOptions {
  /** 底层 S3 预签名原语；缺省新建 {@link S3PresignService}（构造无副作用）。 */
  presigner?: PresignBackend
  /** 单张图片大小上限（字节）；缺省 5MB（`DEFAULT_MAX_IMAGE_BYTES`，需求 22.5）。 */
  maxBytes?: number
}
...
export class UploadService {
  private readonly presigner: PresignBackend
   ...
   */
  async presign(actor: AuthContext, req: PresignRequest): Promise<PresignResult> {
   ...
   */
  private authorize(actor: AuthContext, req: PresignRequest): void {
