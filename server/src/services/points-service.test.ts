...
/** 记录一次 recordLog 调用（含是否携带事务句柄）。 */
interface RecordedLog {
  entry: LogEntry
  handle: unknown
}

class FakeLogRecorder implements PointsLogRecorder {
  logs: RecordedLog[] = []
  ...
  async transaction<T>(fn: (tx: PointsTx) => Promise<T>): Promise<T> {
    ...
    const tx: PointsTx = {
      async lockAccount(userId: string): Promise<LockedPointsAccount | null> {
        if (!self.balances.has(userId)) return null
    ...
    try {
      ...
      // 回滚
      self.balances.clear()
      for (const [k, v] of balancesSnapshot) self.balances.set(k, v)
...
function buildHarness(initial: Record<string, number>): Harness {
