...
import { CONTENT_TYPE_TO_EXT, DEFAULT_MAX_IMAGE_BYTES, PRESIGN_EXPIRY_SECONDS, S3PresignService, buildObjectKey, buildPublicUrl, extFromContentType, } from './s3-presign';
...
const makeSpySigner = () => {
    const calls = [];
    const signer = vi.fn(async (_client, command, options) => {
...
describe('buildPublicUrl', () => {
    it('joins base + /media/<objectKey>', () => {
        expect(buildPublicUrl('avatars/u/x.jpg', 'https://media.example.com')).toBe('https://media.example.com/media/avatars/u/x.jpg');
    });
    it('normalizes a trailing slash on the base and a leading slash on the key', () => {
        expect(buildPublicUrl('/products/p/y.png', 'https://media.example.com/')).toBe('https://media.example.com/media/products/p/y.png');
    });
});
describe('S3PresignService.generatePresignedPut', () => {
    ...
    it('throws when no upload bucket is configured', async () => {
        ...
        await expect(service.generatePresignedPut({ objectKey: 'avatars/u/x.jpg', contentType: 'image/jpeg' })).rejects.toThrow(/UPLOAD_BUCKET/);
    });
});
describe('S3PresignService.buildPublicUrl', () => {
    it('uses the configured media base URL', () => {
        ...
        expect(service.buildPublicUrl('avatars/u/x.jpg')).toBe('https://cdn.example.com/media/avatars/u/x.jpg');
    });
