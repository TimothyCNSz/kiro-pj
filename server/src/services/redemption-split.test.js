...
// ---------------------------------------------------------------------------
function makeLine(id, type, pointsCost, quantity, name = `商品-${id}`) {
    const product = {
        id,
        name,
        type,
...
const totalOf = (lines) => lines.reduce((sum, l) => sum + l.cost, 0);
const sumPoints = (drafts) => drafts.reduce((sum, d) => sum + d.pointsSpent, 0);
// ---------------------------------------------------------------------------
...
describe('splitOrdersByType — Property 15（积分守恒与类型归类）', () => {
    ...
    it('Property 15: 每项正确归类、只为出现的类型建单、Σ pointsSpent = 应付总额', () => {
        fc.assert(fc.property(fc.array(lineArb, { maxLength: 30 }), (raw) => {
            ...
            for (const draft of drafts) {
                const expectedType = draft.type === OrderType.Physical ? ProductType.Physical : ProductType.Virtual;
                // 每个订单项按其商品类型正确归类
