...
import {
  FulfillmentService,
  buildFakeTrackingTimeline,
  TRACKING_REQUIRED_MESSAGE,
  ORDER_TYPE_MISMATCH_MESSAGE,
  type DbOrTx,
...
// ---------------------------------------------------------------------------
// In-memory fakes (no real database)
// ---------------------------------------------------------------------------
...
interface FakeOrder {
  ...
  /** CDK codes linked to this order (consumed at redemption; delivered on ship). */
  cdks: string[]
  cdkDelivered: boolean
}

/** Sentinel handle used to assert the tx flows through to the log recorder. */
const TX_HANDLE = { __tx: true } as unknown as DbOrTx

class FakeFulfillmentRepository implements FulfillmentRepository {
  txRuns = 0
  constructor(private readonly orders: Map<string, FakeOrder>) {}
  ...
  async getOrder(orderId: string, _handle: DbOrTx): Promise<OrderShipInfo | null> {
  ...
  async markVirtualShipped(orderId: string, _handle: DbOrTx): Promise<string[]> {
    const o = this.orders.get(orderId)
    if (!o) throw new Error('order missing')
    o.status = OrderStatus.Shipped
    o.cdkDelivered = true
...
class FakeLogRecorder {
  readonly calls: Array<{ entry: LogEntry; handle: unknown }> = []
  async recordLog(entry: LogEntry, handle?: unknown): Promise<void> {
...
interface Harness {
  ...
  logger: FakeLogRecorder
  orders: Map<string, FakeOrder>
}

function buildHarness(seed: FakeOrder[]): Harness {
  const orders = new Map<string, FakeOrder>(seed.map((o) => [o.id, o]))
...
const physicalOrder = (over: Partial<FakeOrder> = {}): FakeOrder => ({
  id: 'phys-1',
...
const virtualOrder = (over: Partial<FakeOrder> = {}): FakeOrder => ({
  id: 'virt-1',
...
describe('FulfillmentService.shipPhysical (需求 8.2, 8.3, 14.1, 14.3, 14.4)', () => {
  ...
  it('rejects whitespace-only tracking number with TRACKING_REQUIRED (需求 14.3)', async () => {
    ...
    expect((err as HttpError).errorCode).toBe(ErrorCode.TrackingRequired)
    expect((err as HttpError).message).toBe(TRACKING_REQUIRED_MESSAGE)
  ...
  it('rejects non-string tracking number with TRACKING_REQUIRED (需求 14.3)', async () => {
    await expect(
      h.service.shipPhysical('admin-1', 'phys-1', undefined),
    ).rejects.toMatchObject({ errorCode: ErrorCode.TrackingRequired })
    expect(h.orders.get('phys-1')!.status).toBe(OrderStatus.PendingShipment)
  })
  ...
  it('rejects shipping a virtual order via the physical path (type mismatch)', async () => {
    ...
    expect(err).toBeInstanceOf(HttpError)
    expect((err as HttpError).errorCode).toBe(ErrorCode.Validation)
    expect((err as HttpError).message).toBe(ORDER_TYPE_MISMATCH_MESSAGE)
