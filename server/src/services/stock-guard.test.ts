...
describe('assertWithinStock', () => {
  it('does not throw when all items are within stock', () => {
    expect(() =>
      assertWithinStock([{ productId: 'a', requestedQuantity: 1, availableStock: 2 }]),
    ).not.toThrow()
  })
  ...
  it('throws INSUFFICIENT_STOCK on the first sold-out item (需求 5.2)', () => {
    ...
    try {
      ...
      expect((err as { errorCode: string }).errorCode).toBe(ErrorCode.InsufficientStock)
    }
  })

  it('throws INSUFFICIENT_STOCK when a quantity exceeds stock (需求 6.3)', () => {
    expect(() =>
      assertWithinStock([{ productId: 'a', requestedQuantity: 5, availableStock: 3 }]),
    ).toThrow()
  })
})
