...
class FakeAdminUserRepository implements AdminUserRepository {
  ...
  lastPagination: PaginationParams | null = null

  constructor(private readonly all: AdminUserRow[]) {}
  ...
  async listUsers(keyword: string, pagination: PaginationParams): Promise<AdminUserPage> {
    ...
    const matched =
      q.length === 0 ? this.all : this.all.filter((u) => u.email.toLowerCase().includes(q))
...
function row(overrides: Partial<AdminUserRow> & { userId: string; email: string }): AdminUserRow {
  return {
    role: Role.Employee,
...
describe('AdminUserService.listUsers', () => {
  ...
  it('defensively drops rows whose email does not match, regardless of repo output (Property 37 不变式)', async () => {
    ...
    const buggyRepo: AdminUserRepository = {
      async listUsers() {
        return {
          rows: [
