...
const employee = { userId: 'emp-1', role: Role.Employee };
...
async function expectErrorCode(p, code) {
...
describe('UploadService.presign — 权限校验 (需求 22.1)', () => {
    it('rejects product upload by a non-admin with FORBIDDEN', async () => {
        ...
        await expectErrorCode(service.presign(employee, {
            purpose: 'product',
            targetId: 'prod-9',
...
describe('UploadService.presign — 格式校验 (需求 22.2, 22.4)', () => {
    ...
    it.each(rejected)('rejects unsupported content type "%s" with UNSUPPORTED_IMAGE_TYPE', async (ct) => {
        const { service } = buildService();
        await expectErrorCode(service.presign(admin, { purpose: 'product', targetId: 'p1', contentType: ct, size: 1000 }), ErrorCode.UnsupportedImageType);
    });
});
// ---------------------------------------------------------------------------
...
describe('UploadService.presign — 大小校验 (需求 22.3, 22.5)', () => {
    ...
    it('rejects a size just over 5MB with IMAGE_TOO_LARGE', async () => {
        const { service } = buildService();
        await expectErrorCode(service.presign(admin, {
            purpose: 'product',
            targetId: 'p1',
            contentType: 'image/png',
    ...
    it.each([0, -1])('rejects a non-positive size %s with IMAGE_TOO_LARGE', async (size) => {
        const { service } = buildService();
        await expectErrorCode(service.presign(admin, { purpose: 'product', targetId: 'p1', contentType: 'image/png', size }), ErrorCode.ImageTooLarge);
    });
    it('honors a custom maxBytes bound', async () => {
        const presigner = new FakePresigner();
        const service = new UploadService({ presigner, maxBytes: 100 });
        await expectErrorCode(service.presign(admin, { purpose: 'product', targetId: 'p1', contentType: 'image/png', size: 101 }), ErrorCode.ImageTooLarge);
    });
});
// ---------------------------------------------------------------------------
// Ordering + delegation
...
describe('UploadService.presign — 校验顺序与委托', () => {
    ...
    it('does not sign a URL when validation fails', async () => {
        const { service, presigner } = buildService();
        await expectErrorCode(service.presign(admin, { purpose: 'product', targetId: 'p1', contentType: 'image/gif', size: 1 }), ErrorCode.UnsupportedImageType);
        expect(presigner.generateCalls).toEqual([]);
    });
