...
import { CartService, computeCartView, } from './cart-service';
...
class FakeCartRepository {
    ...
    async listLines(cartId) {
        const map = this.lines.get(cartId) ?? new Map();
        const rows = [];
        ...
        return rows.sort((a, b) => a.name.localeCompare(b.name) || a.productId.localeCompare(b.productId));
    }
...
describe('CartService', () => {
    ...
    it.each([0, -1, 1.5, Number.NaN])('rejects invalid add quantity %p with VALIDATION', async (qty) => {
        await expect(service.addItem(USER, 'p1', qty)).rejects.toMatchObject({
