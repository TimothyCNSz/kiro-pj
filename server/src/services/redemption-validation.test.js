...
import { buildRedemptionPreValidate, validateRedemption, assertAddressForPhysical, assertLinesWithinStock, containsPhysicalLine, isValidAddress, toStockCheckItems, } from './redemption-validation';
...
describe('pure helpers', () => {
    ...
    it('isValidAddress validates presence and non-empty required fields', () => {
        ...
        expect(isValidAddress({ recipient: '', phone: '1', detail: 'd' })).toBe(false);
        expect(isValidAddress({ recipient: 'a', phone: ' ', detail: 'd' })).toBe(false);
        expect(isValidAddress({ recipient: 'a', phone: '1', detail: '  ' })).toBe(false);
    });
