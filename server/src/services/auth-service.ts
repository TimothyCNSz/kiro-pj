...
import { DrizzleSessionService, type SessionService } from './session-service'
...
/**
 * 认证用户存储抽象。默认实现基于 Drizzle（见 `DrizzleAuthStore`），
 * 测试可注入内存替身。
 */
export interface AuthStore {
  /** 按（规范化后的）邮箱精确查找；不存在返回 null。 */
  findByEmail(email: string): Promise<AuthUserRecord | null>
  /**
   * 在单事务内创建「待验证」员工账号与其零余额积分账户，返回新用户 id。
   * 邮箱唯一冲突应抛出携带 `ErrorCode.EmailTaken` 的 `HttpError`。
   */
  createEmployeeWithPointsAccount(input: {
    email: string
    passwordHash: string
...
export class JwtTokenSigner implements TokenSigner {
  ...
  constructor(options: { secret?: string; expiresInSeconds?: number } = {}) {
...
export interface AuthServiceOptions {
  /** 用户存储（缺省 Drizzle）。 */
  store?: AuthStore
  /** 邮箱验证服务（register 成功后 `issue`）。 */
  emailVerificationService: Pick<EmailVerificationService, 'issue'>
  /** 会话服务（缺省 Drizzle）。 */
  sessionService?: SessionService
  /** JWT 签发（缺省基于 JWT_SECRET）。 */
  tokenSigner?: TokenSigner
  /** 公司邮箱域名白名单（缺省取自 `COMPANY_EMAIL_DOMAINS`）。 */
  companyEmailDomains?: readonly string[]
  /** 口令哈希函数（缺省 scrypt）。 */
...
/** register 结果：含发送失败标记（设计「发送失败不回滚账号」）。 */
export interface RegisterResult {
  userId: string
  status: typeof AccountStatus.PendingVerification
  /** 账号已创建但验证邮件发送失败时为 true（需求 1.4；HTTP 202）。 */
...
export class AuthService {
  ...
  private readonly emailVerificationService: Pick<EmailVerificationService, 'issue'>
  private readonly sessionService: SessionService
  private readonly tokenSigner: TokenSigner
  private readonly companyEmailDomains: readonly string[]
