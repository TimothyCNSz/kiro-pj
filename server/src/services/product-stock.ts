 ...
 */
export function isSoldOut(availableStock: number): boolean {
  return !(availableStock > 0)
}

/** 库存解析所需的最小商品视图（只依赖 id/type/stock 三字段）。 */
export interface StockProductView {
  /** 商品 id（虚拟商品据此计数可用 CDK）。 */
  id: string
  /** 商品类型（决定库存来源：虚拟→可用 CDK 数，实物→stock 字段）。 */
  type: ProductType
  /** 实物商品的库存字段；虚拟商品该字段仅作缓存，不作为权威来源。 */
  stock: number
}

/**
 * 可用 CDK 计数接缝：返回某虚拟商品状态为 `available` 的 CDK 数量。
 * 默认实现基于 Drizzle（见 {@link DrizzleCdkCounter}），测试可注入内存替身。
 */
export interface CdkCounter {
  /** 统计给定商品的可用（未使用）CDK 数量（需求 5.1、12.2）。 */
  countAvailable(productId: string): Promise<number>
}

/** 基于 Drizzle 的默认可用 CDK 计数实现。 */
export class DrizzleCdkCounter implements CdkCounter {
...
/**
 * 库存解析器接缝（供 CatalogService 任务 5.1 注入）。
 * 输入商品视图，输出其可兑换库存数量（虚拟商品派生自可用 CDK 数）。
 */
export interface StockResolver {
  /** 解析单个商品的可兑换库存数量。 */
  resolve(product: StockProductView): Promise<number>
}
 ...
 */
export class ProductStockService implements StockResolver {
  private readonly cdkCounter: CdkCounter

  constructor(options: { cdkCounter?: CdkCounter; db?: Database } = {}) {
    this.cdkCounter = options.cdkCounter ?? new DrizzleCdkCounter(options.db ?? defaultDb)
  }

  /** 虚拟商品的可兑换库存 = 可用 CDK 数量（需求 5.1、12.2）。 */
  async getVirtualStock(productId: string): Promise<number> {
  ...
  async isSoldOut(product: StockProductView): Promise<boolean> {
