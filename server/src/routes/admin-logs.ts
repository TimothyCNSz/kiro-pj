...
import { paginated, type PaginatedData, type PaginationParams } from '../lib/api'
import { adminGuard, createAuthMiddleware } from '../middleware/auth'
import { LogService, type OperationLogView } from '../services/log-service'
import { parsePagination } from './products'

/** 操作日志列表成功提示。 */
export const LIST_LOGS_OK_MESSAGE = '操作日志列表'

/** 操作日志查询服务接口（以结构化接口耦合，便于注入替身）。 */
export interface LogQueryService {
  listLogs(pagination: PaginationParams): Promise<PaginatedData<OperationLogView>>
}

/** `createAdminLogsRouter` 依赖（全部可注入以支持无副作用测试）。 */
export interface AdminLogsRouterDependencies {
  logService: LogQueryService
 ...
 */
export function createAdminLogsRouter(deps: AdminLogsRouterDependencies): Router {
  const router = Router()
  ...
  router.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      res.json(
        paginated(result.list, { total: result.total, page: result.page, pageSize: result.pageSize }, LIST_LOGS_OK_MESSAGE),
      )
    }),
  )
 ...
 */
export function buildDefaultAdminLogsRouter(): Router {
