...
/** 兑换执行器接口（以结构化接口耦合，便于注入替身）。由 RedemptionService 实现。 */
export interface RedemptionExecutor {
  checkout(
    userId: string,
...
/** 购物车读取接口：结算时汇总当前员工购物车的商品项（需求 7.1）。 */
export interface CartItemsReader {
  getCart(userId: string): Promise<{ items: ReadonlyArray<{ productId: string; quantity: number }> }>
}

/** `createRedemptionsRouter` 依赖（全部可注入以支持无副作用测试）。 */
export interface RedemptionsRouterDependencies {
  /** 结算执行器（来自购物车，成功后清理购物车已兑项，需求 7.5）。 */
  checkoutService: RedemptionExecutor
  /** 立即兑换执行器（单件，不清理购物车）。 */
  instantService: RedemptionExecutor
  /** 购物车读取器（结算时汇总条目）。 */
  cartReader: CartItemsReader
  /** 保护全部端点的认证中间件（兑换需登录，需求 1.15）。 */
  authMiddleware: RequestHandler
}

/** 兑换响应信封的数据部分（面向前端的兑换结果摘要）。 */
 ...
 */
function normalizeInstantQuantity(value: unknown): number {
...
/** 将兑换事务结果映射为面向前端的响应摘要。 */
function toRedemptionResponse(result: CheckoutResult): RedemptionResponse {
  return {
 ...
 */
export function createRedemptionsRouter(deps: RedemptionsRouterDependencies): Router {
  const router = Router()
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const items: RedeemItem[] = cart.items.map((item) => ({
 ...
 */
export function buildDefaultRedemptionsRouter(
  overrides: { cart?: Partial<CartServiceDependencies> } = {},
): Router {
