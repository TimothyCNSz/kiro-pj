...
const VALID_PURPOSES = new Set(['avatar', 'product']);
/** 从请求体解析并校验 presign 入参；结构非法以 VALIDATION(422) 拒绝。 */
function parsePresignRequest(body) {
    const b = (body ?? {});
    ...
    return { purpose: purpose, targetId, contentType, size };
}
/**
 ...
 * 构造无副作用（AWS 客户端惰性建立）。
 */
export function buildDefaultUploadsRouter(overrides = {}) {
