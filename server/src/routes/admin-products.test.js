...
import { createAdminProductsRouter, PRODUCT_CREATED_MESSAGE, PRODUCT_UPDATED_MESSAGE, PRODUCT_STATUS_UPDATED_MESSAGE, } from './admin-products';
...
import { ProductValidationError, } from '../services/admin-product-service';
// ---------------------------------------------------------------------------
// Fakes
// ---------------------------------------------------------------------------
const sampleProduct = (over = {}) => ({
    id: 'prod-1',
    name: 'p',
    imageUrl: null,
    description: '',
...
class FakeAdminProductService {
    createResult = sampleProduct();
    createError = null;
    updateResult = sampleProduct({ name: 'renamed' });
    setStatusResult = sampleProduct({ status: ProductStatus.Listed });
    created = [];
    updated = [];
    statusCalls = [];
    async create(input) {
...
const ADMIN = { sub: 'admin-1', sid: 'sid-admin', role: Role.Admin };
const EMPLOYEE = { sub: 'emp-1', sid: 'sid-emp', role: Role.Employee };
...
const asAdmin = (r) => r.set('Authorization', 'Bearer admin-tok');
// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
describe('POST /admin/products', () => {
    ...
    it('creates a product and returns 201 (需求 12.1)', async () => {
        ...
        const res = await asAdmin(request(h.app)
            .post('/admin/products')
...
describe('PUT /admin/products/:id', () => {
    ...
    it('updates a product and returns 200 (需求 12.3)', async () => {
        ...
        const res = await asAdmin(request(h.app).put('/admin/products/p1').send({ name: 'renamed' }));
    ...
    it('returns 404 when the product does not exist', async () => {
        ...
        const res = await asAdmin(request(h.app).put('/admin/products/missing').send({ name: 'x' }));
...
describe('PATCH /admin/products/:id/status', () => {
    ...
    it('switches product status and returns 200 (需求 12.4)', async () => {
        ...
        const res = await asAdmin(request(h.app).patch('/admin/products/p1/status').send({ status: ProductStatus.Listed }));
        expect(res.status).toBe(200);
