...
/** 积分调整服务接口（以结构化接口耦合，便于注入替身）。 */
export type PointsAdjustService = Pick<PointsService, 'adjust' | 'batchAdjust'>

/** `createAdminPointsRouter` 依赖（全部可注入以支持无副作用测试）。 */
export interface AdminPointsRouterDependencies {
  pointsService: PointsAdjustService
...
function requireDelta(value: unknown): number {
...
function requireUserIds(value: unknown): string[] {
...
function extractNote(value: unknown): string | undefined {
 ...
 */
export function createAdminPointsRouter(deps: AdminPointsRouterDependencies): Router {
  const router = Router()
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      res.json(success<PointsAccountView>(account, ADJUST_OK_MESSAGE))
    }),
  )
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      res.json(success<BatchAdjustResult>(result, BATCH_ADJUST_OK_MESSAGE))
    }),
  )
 ...
 */
export function buildDefaultAdminPointsRouter(): Router {
