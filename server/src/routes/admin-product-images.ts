...
/** 图集命令服务接口（以结构化接口耦合，便于注入替身）。 */
export interface ProductImageCommandService {
  addImage(productId: string, objectKey: string, url: string): Promise<ProductImage>
  setPrimary(productId: string, imageId: string): Promise<boolean>
  removeImage(productId: string, imageId: string): Promise<boolean>
}

/** `createAdminProductImagesRouter` 依赖（全部可注入以支持无副作用测试）。 */
export interface AdminProductImagesRouterDependencies {
  service: ProductImageCommandService
...
export function createAdminProductImagesRouter(
  deps: AdminProductImagesRouterDependencies,
): Router {
  const router = Router()
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      const body = (req.body ?? {}) as { objectKey?: unknown; url?: unknown }
 ...
 */
export function buildDefaultAdminProductImagesRouter(): Router {
