...
import { createCartRouter, type CartCommandService } from './cart'
...
const emptyCart: CartView = { items: [], totalPoints: 0 }

class FakeCartService implements CartCommandService {
  view: CartView = emptyCart
  /** Optional error to throw from the next call (to exercise error mapping). */
  nextError: HttpError | null = null

  readonly getCalls: string[] = []
  readonly addCalls: Array<{ userId: string; productId: string; quantity: number }> = []
  readonly updateCalls: Array<{ userId: string; productId: string; quantity: number }> = []
  readonly removeCalls: Array<{ userId: string; productId: string }> = []
