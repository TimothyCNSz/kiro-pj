...
/** CDK 命令服务接口（以结构化接口耦合，便于注入替身）。 */
export interface CdkCommandService {
  addCdks(
    productId: string,
    codes: unknown,
  ): Promise<{ added: number; availableStock: number }>
}

/** `createAdminCdksRouter` 依赖（全部可注入以支持无副作用测试）。 */
export interface AdminCdksRouterDependencies {
  cdkService: CdkCommandService
 ...
 */
export function createAdminCdksRouter(deps: AdminCdksRouterDependencies): Router {
  const router = Router()
  ...
  router.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      const codes = (req.body as { codes?: unknown } | undefined)?.codes
 ...
 */
export function buildDefaultAdminCdksRouter(): Router {
