...
import { AlertService, type LowStockAlertView } from '../services/alert-service'

/** 低库存提醒列表成功提示。 */
export const LIST_LOW_STOCK_OK_MESSAGE = '低库存提醒列表'

/** 低库存查询服务接口（以结构化接口耦合，便于注入替身）。 */
export interface AlertQueryService {
  listLowStock(): Promise<LowStockAlertView[]>
}

/** `createAdminAlertsRouter` 依赖（全部可注入以支持无副作用测试）。 */
export interface AdminAlertsRouterDependencies {
 ...
 */
export function createAdminAlertsRouter(deps: AdminAlertsRouterDependencies): Router {
  const router = Router()
 ...
 */
export function buildDefaultAdminAlertsRouter(): Router {
