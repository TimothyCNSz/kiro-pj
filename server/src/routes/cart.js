 ...
 */
export function createCartRouter(deps) {
    const router = Router();
    ...
    router.post('/items', asyncHandler(async (req, res) => {
        ...
        const cart = await deps.cartService.addItem(userId, productId, body.quantity);
    ...
    router.patch('/items/:productId', asyncHandler(async (req, res) => {
        const userId = requireUserId(req);
        const body = (req.body ?? {});
        const cart = await deps.cartService.updateItem(userId, req.params.productId, body.quantity);
        res.json(success(cart));
    }));
    // 移除某条目（需求 6.4）。
 ...
 * 构造无副作用（数据库连接惰性建立）。
 */
export function buildDefaultCartRouter(overrides = {}) {
