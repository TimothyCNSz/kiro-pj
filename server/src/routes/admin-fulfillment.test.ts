...
import {
  ...
  type FulfillmentCommandService,
  SHIP_PHYSICAL_OK_MESSAGE,
  SHIP_VIRTUAL_OK_MESSAGE,
...
import type { ShipPhysicalResult, ShipVirtualResult } from '../services/fulfillment-service'

// ---------------------------------------------------------------------------
// Fakes
// ---------------------------------------------------------------------------

class FakeFulfillmentService implements FulfillmentCommandService {
  readonly physicalCalls: Array<{ adminId: string; orderId: string; trackingNo: unknown }> = []
  readonly virtualCalls: Array<{ adminId: string; orderId: string }> = []
  physicalResult: ShipPhysicalResult | null = {
    orderId: 'phys-1',
    status: OrderStatus.Shipped,
    trackingNo: 'SF123',
    tracking: { trackingNo: 'SF123', carrier: 'demo', nodes: [{ status: 's', description: 'd' }] },
  }
  virtualResult: ShipVirtualResult | null = {
    orderId: 'virt-1',
    status: OrderStatus.Shipped,
    cdks: ['CDK-A'],
  }
  physicalError: unknown = null
  virtualError: unknown = null

  async shipPhysical(adminId: string, orderId: string, trackingNo: unknown) {
    this.physicalCalls.push({ adminId, orderId, trackingNo })
    if (this.physicalError) throw this.physicalError
    return this.physicalResult
  }

  async shipVirtual(adminId: string, orderId: string) {
    this.virtualCalls.push({ adminId, orderId })
    if (this.virtualError) throw this.virtualError
