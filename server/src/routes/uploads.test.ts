...
import { createUploadsRouter, type UploadCommandService } from './uploads'
...
import type { AuthContext, PresignRequest, PresignResult } from '../services/upload-service'
...
const sampleResult: PresignResult = {
...
class FakeUploadService implements UploadCommandService {
  result: PresignResult = sampleResult
  nextError: HttpError | null = null
  readonly calls: Array<{ actor: AuthContext; req: PresignRequest }> = []
...
const EMP_PAYLOAD: AuthTokenPayload = { sub: 'emp-1', sid: 'sid-emp', role: Role.Employee }
...
const auth = (r: request.Test) => r.set('Authorization', 'Bearer tok-emp')
